...
func ChaCha(key []byte, nonce []byte, blockCount uint32) *ChaCha_ {
	return &ChaCha_{
...
func (cc *ChaCha_) Cipher(text []byte) []byte {
...
func (cc *ChaCha_) block(state []uint32) []uint32 {
...
func (cc *ChaCha_) initState(blockCount uint32) []uint32 {
...
func (cc *ChaCha_) printWords(state []uint32) {
	for i, v := range state {
		if i%4 == 0 {
			fmt.Printf("\n")
		}
